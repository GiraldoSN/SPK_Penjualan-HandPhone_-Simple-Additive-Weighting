                  ...
                  <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                      ...
                      foreach ($matrix as $id => $values) {
                        // Ambil nilai minimum dan maksimum dari matriks X
                        $minC1 = min(array_column($matrix, 'C1'));
                        $maxC2 = max(array_column($matrix, 'C2'));
                        $maxC3 = max(array_column($matrix, 'C3'));
                        $maxC4 = max(array_column($matrix, 'C4'));
                        $maxC5 = max(array_column($matrix, 'C5'));

                        // Hitung nilai R sesuai rumus
                        $R = array(
                          'C1' => ($minC1 != 0) ? $minC1 / $values['C1'] : 0, // Cost
                          'C2' => ($maxC2 != 0) ? $values['C2'] / $maxC2 : 0, // Benefit
                          'C3' => ($maxC3 != 0) ? $values['C3'] / $maxC3 : 0, // Benefit
                          'C4' => ($maxC4 != 0) ? $values['C4'] / $maxC4 : 0, // Benefit
                          'C5' => ($maxC5 != 0) ? $values['C5'] / $maxC5 : 0  // Benefit
                        );
                        echo "<tr>
                                  <td>A{$id}</td>
                                  <td>" . round($R['C1'], 2) . "</td>
